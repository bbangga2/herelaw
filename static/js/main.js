...
function getNextQuestion(){
  ...
  const hasParentAnswerList = parentQuestionList.filter(element => !!element.parentAnswer);
...
$(function () {
  ...
  $("#next-btn").click(function () {
    if(currentQuestionCursor < answerList.length - 1 && answerList[currentQuestionCursor].answer !== getAnswerValue(answerList[currentQuestionCursor].questionId)){
      const confirmResult = confirm('값이 바뀌면 뒤의 질문들이 바뀔 수 있습니다.');
      if(confirmResult){
    ...
    if (currentQuestionCursor >= answerList.length - 1) {
      ...
      if (newQuestion) {
        ...
        console.log('제출에 진입')
        const submitButton = $('<button>').text('제출하기').attr('id', 'submit-button').prop('disabled', false);
