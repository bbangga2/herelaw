...
def summary():
    #res_gpt = None
    #if request.args.get('res_gpt'):
    res_gpt = session.get('res_gpt')
    print("--------------------------------------")
    #print(res_gpt)
    # return render_template('summary.html', res_gpt=res_gpt)
    print(res_gpt)
    return render_template('summary.html', res_gpt=res_gpt)
...
def submit_survey():
    ...
    data = request.json['location']  # json 데이터와 파일을 함께 처리하려면 request.form으로 받아서 json화 되어있는 location 로드하기
    ...
    session['res_gpt'] = res_gpt

    return redirect('/summary')
    #return redirect(url_for('summary',res_gpt=res_gpt))
